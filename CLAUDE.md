...
- `10_chromadb/` - ChromaDB vector database storage
...
### Source Modules (`src/`)
- **Processing modules**: `p01`, `p03`, `p05`, `p06`, `p10`, `p11`, `p12`, `p14` (organized by pipeline stage)
- `p10_Chroma_dbm.py`: ChromaDB wrapper (`ChromaEmbeddingStore` class)
- **GraphSAGE variants**:
  - `p14_GraphSAGE_v03_dynamic_chrdb.py`: Dynamic training with ChromaDB
  - `p14_GraphSAGE_v04_dynamic_optuned.py`: Dynamic training with Optuna
  - `p14_GraphSAGE_v05_static_optuned.py`: Static training with Optuna
  - `p14_GraphSAGE_u_loss.py`, `p14_GraphSAGE_v_evaluate.py`, `p14_GraphSAGE_v_visual.py`
- `imports/`: Import modules for notebook integration

### Notebooks (`notebooks/`)
- `n01-n15`: Analysis notebooks corresponding to pipeline stages
